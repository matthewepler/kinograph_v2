...
void loop() {
  ...
  analogWrite(s_inputB, s_output);  

  Serial.print(f_output);
  Serial.print(" : ");
  Serial.println(t_output);  
  Serial.print(" : ");
  Serial.println(s_output);
...
void initMotors() {
  ...
  // pinMode(c_enable, OUTPUT); // comment out if using switch

  // toggle enable signal to reset any error states
  // comment out if using switch
  // digitalWrite(f_enable, LOW);
  // digitalWrite(t_enable, LOW);
